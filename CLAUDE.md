...
└── supply/
    ...
    └── admin/
        ├── page.tsx                # 4. 役員専用・ワンクリック発注/納品管理画面（承認待ち・納品待ちのみ）
        └── history/page.tsx        # 【新規】4-2. 過去歴専用画面（納品済み・却下を月別グルーピング表示）
```
...
- **UI構成**:
    - タスク消化型UIを維持し、画面上部に「承認待ち」「納品待ち」の2エリアを配置。
    - 「納品済み」「却下」は増え続けるデータのため、このページでは取得・表示せず、別画面（画面4-2）へ切り出す。ヘッダーに **[🗂️ 過去歴]** ボタンを配置し画面4-2へ遷移する。
- **各エリアの仕様**:
    - **承認待ち一覧**:
        - 従来のUI（[購入先ページを開く] ボタン、[承認する] ボタン）に加え、**[却下]** ボタンを追加。却下時は理由入力モーダル（`RejectReasonModal`）を表示。
    - **納品待ち一覧**:
        - 本部へ届いた備品を役員側でも完了できるよう、**[納品済みにする]** ボタンを配置。

#### 画面4-2: 【新規】過去歴専用画面 (`app/supply/admin/history/page.tsx`)
...
- **月グルーピングの基準**:
    ...
    - 却下: `rejected_at` の年月
- **役員専用**: 画面4と同じロール判定（`role !== '役職'` は `/supply` へリダイレクト）を適用。
...
└── supply/
    ...
    └── admin/
        ...
        └── history/page.tsx        # 4-2. 過去歴専用画面（納品済み・却下の月別表示）
```
